...
[numStim,numBlock,blockInd,correctResp,SOARef,rhythmicSOA,numTrial,~,~,indivAmp] = varIntro;%output indivAmp from staircase
...
[~,~,condMat,~]    = condMatCreator(blockInd,numBlock,numTrial,numStim,SOARef,rhythmicSOA,faceRand,correctResp,indivAmp); %Conditions Matrix
...
% Start KbQueu routine -- check the modification for Nata box
...
for blk = 1 %(numBlock*length(blockInd))  %total nr of blocks = block types (3) * repetition of each block
  ...
  %Start an event 
        ...
        if frmsInBlk == condMat(trilAud,17) 
            audOnset = GetSecs; %should be removed after debugging
            triggerSend(trigHandle,trigAdd,trigAud,MEGLab); 
            SCTriggerSound(condMat(trilAud,10));
